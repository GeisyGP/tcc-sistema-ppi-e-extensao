...
import { useUniqueProject } from "@/app/(main)/projects/[id]/hooks/use-unique-project"
...
export function DeliverableModal({ isOpen, deliverable, projectId, onClose, onSave, onDelete }: DeliverableModalProps) {
    const isEditMode = !!deliverable
    const { userId, userRole } = useRole()
    ...
    useEffect(() => {
        ...
        if (!isOpen || !isEditMode || !deliverable) {
            setShouldRender(true)
            return
        }

        const subjectId = deliverable?.subjectId
        if (!subjectId || userRole === UserRole.COORDINATOR) {
            setShouldRender(true)
            return
        }
        ;(async () => {
            ...
            const allowed = subject.teachers.some((u) => u.id === userId)
            if (!allowed) {
    ...
    }, [isOpen, isEditMode, deliverable, onClose, userRole, userId])
                        ...
                        <div className="space-y-3">
                            {isEditMode && (
                                <p className="mt-4 text-sm text-gray-600 leading-snug border-l-2 border-green-400 pl-2">
                                    As entregas realizadas pelos grupos podem ser consultadas acessando o grupo
                                    correspondente na página do projeto.
                                </p>
                            )}
